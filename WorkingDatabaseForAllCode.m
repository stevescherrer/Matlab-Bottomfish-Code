...
%%%This Dead Fish bit didn't work. silenced in version 1.7
...
%DeadFishDates=round(min(DeadFish(:,2)+.5))-1:round(max(DeadFish(:,2)+.5)-1); %for determining days of study period from earliest detection date to latest
%DeadFishTimeDateMatrix=zeros(round(max(DeadFish(:,2))-min(DeadFish(:,2))+.5),length(DeadFishTimeBins(1,:))); %makes a matrix 25 columns long and length of number of days in study
...
Date=(datenum(2012,08,18)-min(Date))+Date;
...
Ehu=BottomFish(BottomFish(:,5)==1,:);
Opakapaka=BottomFish(BottomFish(:,5)==2,:);
Kalekale=BottomFish(BottomFish(:,5)==5,:);
%Onaga=BottomFish(BottomFish(:,5)==___,:);

%%%%END ADDED SECION April 28

csvwrite('BottomFish.csv',BottomFish);
csvwrite('Ehu.csv',Ehu);
csvwrite('Opakapaka.csv',Opakapaka);
csvwrite('KaleKale.csv',KaleKale);
%csvwrite('Onaga.csv',Onaga);
