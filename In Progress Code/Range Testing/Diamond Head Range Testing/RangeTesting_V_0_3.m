...
%%Assigning Ping interval For these tags mean interval was 180 seconds
ValueOfASecond=1/(24.*60.*60); %for converting seconds to date time

for i=1:length(TagsAndReceiverData); %indexes TagString1
    TagsAndReceiverData(i,5)=180.*ValueOfASecond; %determines difference between detections and selects smallest detection interval 
end
...
%%%VERSION HISTORY
%%V_0_3 changed ping interval from minimum detection interval to mean detection
    %%interval
