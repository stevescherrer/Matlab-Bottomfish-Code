...
Addendum=nan(length(MaxSpeedsBySpecies(:,1)),1); %%Padding for MaxSpeedBySpeciesMatrix to add distance between recievers
MaxSpeedsBySpecies=[MaxSpeedsBySpecies,Addendum]; 
clear Addendum

%%Determining Reciever Lat/Long at Time of Detecion
for i=1:length(MaxSpeedsBySpecies);
    for a=1:length(RecieverDates);
        if MaxSpeedsBySpecies(i,5)==ReceiverDates(a,2) && MaxSpeedsBySpecies(i,3)>=ReceiverDates(a,3) && MaxSpeedsBySpecies(i,3)<=ReceiverDates(a,4);
            FirstReciever=RecieverDates(a,:);
        end
        if MaxSpeedsBySpecies(i,6)==ReceiverDates(a,2) && MaxSpeedsBySpecies(i,4)>=ReceiverDates(a,3) && MaxSpeedsBySpecies(i,4)<=ReceiverDates(a,4);
            ...
            MaxSpeedsBySpecies(i,7)=arclen;
    end
end

%%Determining Maximum Swimming Speed and displaying it
    disp 'Ehu'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==1,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==1,2)
    disp 'Opakapaka'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==2,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==2,2)
    disp 'Dogfish'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==3,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==3,2)
    disp 'Ges'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==4,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==4,2)
    disp 'Kale'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==5,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==5,2)
    disp 'Sandbar'
    MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==6,7)/ MaxSpeedsBySpecies(MaxSpeedsBySpecies(:,1)==6,2)
