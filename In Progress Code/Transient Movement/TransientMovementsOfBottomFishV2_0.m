...
%%%%into and out of the BAFR MPA in 1 week intervals. 
...
%%%%called BAFRMovements. NaNs indicate a period with no detections. A zero
...
BAFRMovements=zeros(length(Tags),length(Bins));
    ...
    for b=1:length(Bins)-1
        ...
        for F=1:f
            if fish(F,2)>=Bins(b) && fish(F,2)<Bins(b+1)
                BAFRMovements(i,b)=BAFRMovements(i,b)+1;
            end
        end
    end
end
...
BAFRMovements(BAFRMovements==0)=NaN;  %%%Zeros were used in previous database to allow counting. Now changing all zeros to NaNs.
...
for i=1:length(Tags)
    ...
    [y,x]=size(Subset);
    for s=2:y
        if Subset(s,5)==Subset(s-1,5) %%if monitor at which detection occurred is same as the one that proceeds it
...
%%Building a ghost matrix same dimensions as BAFRMovements but with NaNs to fill in with
%%zeros for bins where detections occur.
BAFRMovementsGhost=NaN(size(BAFRMovements));
...
for i=1:length(Tags)
    ...
    BAFRMovementsGhost(i,:)=IndividualLine;
end

clearvars i Subset IndividualLine y x b a DetectionsWithoutMovements

%%%%Consolodating BAFRMovements and BAFRMovementsGhost so detections in
%%%%which no movements occur are indicated by zeros and bins with no
%%%%detections are indicated by NaNs

[y,x]=size(BAFRMovements); %%BAFRMovements and BAFRMovementsGhost are the same size

for i=1:y
    for a=1:x
        if isnan(BAFRMovements(i,a))==1;
            BAFRMovements(i,a)=BAFRMovementsGhost(i,a);
        end
    end
end

clearvars i a x y BAFRMovementsGhost Tags

save 'BottomFish'; save 'BAFRMovements'
