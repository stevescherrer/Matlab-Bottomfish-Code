%%For Determining Placement of Receiver line around BRFA E
...
LowerDepthLimit=-500; %in meters
HeightOfReceiverOffBottom=30; %in meters
Receiver12Percent=250; %Radius of receiver detection sphere where 12.5 percent of pings are picked up from range testing data
Receiver25Percent=200; %Radius of receiver detection sphere where 25 percent of pings are picked up from range test data
NumberOfReceiversToPlace=11;
AcceptableDetectionDistanceFromCenterOfReceiver=150; %%Distance Away from center of Receiver that tags must still be heard
...
%%Column 1=sequential receiver number
%%Column 2=Distance between subsequent receivers
%%Column 3=Depth of that placement%%For Determining Placement of Receiver line
...
RadiusOfOverlappingSphere=(sqrt((Receiver12Percent.^2)-((FenceLine(R,2)/2)^2)));
LengthOfAcceptableDetection=2*sqrt(RadiusOfOverlappingSphere^2-AcceptableDetectionDistanceFromCenterOfReceiver^2);
if isreal(LengthOfOverlappingSphere)==0;
    disp ('Insufficient receivers to place a fence')
else
    if RadiusOfOverlappingSphere<=AcceptableDetectionDistanceFromCenterOfReceiver;
        disp('Insufficient receivers to place a fence')
